...
import { ALUMNO_PAGE_HEADER, ALUMNO_PAGE_CARDS } from "@/data/portal-cards"
      ...
      <main className="container mx-auto px-4 lg:px-8 py-12">
        <div className="mb-10">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h2 className="text-3xl font-bold text-foreground">{ALUMNO_PAGE_HEADER.title}</h2>
          </div>
          <p className="text-muted-foreground text-lg ml-7 text-pretty">
            {ALUMNO_PAGE_HEADER.subtitle}
          </p>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {ALUMNO_PAGE_CARDS.map((card, index) => (
            <DashboardCard
              key={index}
              title={card.title}
              icon={card.icon}
